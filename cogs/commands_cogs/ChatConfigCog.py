...
from ..common import current_language, prompts, instruc_config, message_history
...
class ChatConfigCog(commands.Cog):
    ...
    @commands.hybrid_command(
        name="toggleactive", description=current_language["toggleactive"]
    )
    ...
    async def toggleactive(
        ...
        persona: discord.app_commands.Choice[str] = prompts[instruc_config],
    ):
    ...
    async def clear(self, ctx: commands.Context):
        ...
        try:
            ...
            await ctx.send(
                f"⚠️ There is no message history to be cleared \n ```{e}```",
                delete_after=2,
            )
            return
